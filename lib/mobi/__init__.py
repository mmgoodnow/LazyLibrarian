...
class MobiTests(unittest.TestCase):
  def setUp(self):
    self.mobitest = Mobi("unittests/testdata/CharlesDarwin.mobi");
  def testParse(self):
    self.mobitest.parse();
  ...
  def testAuthorTitle(self):
    self.mobitest.parse()
    self.assertEqual(self.mobitest.author(), b'Charles Darwin')
    self.assertEqual(self.mobitest.title(), b'The Origin of Species by means '+
        b'of Natural Selection, 6th Edition')
