...
def processAlternate(source_dir=None, library='eBook'):
    ...
    try:
        ...
        flist = os.listdir(makeUTF8bytes(source_dir)[0])
        ...
        if library == 'eBook':
            ...
            if not new_book:
                ...
                for f in os.listdir(makeUTF8bytes(source_dir)[0]):
                    f = makeUnicode(f)
...
def move_into_subdir(sourcedir, targetdir, fname, move='move'):
    ...
    cnt = 0
    list_dir = os.listdir(makeUTF8bytes(sourcedir)[0])
...
def processDir(reset=False, startdir=None, ignoreclient=False):
    ...
    try:
        ...
        for download_dir in dirlist:
            try:
                downloads = os.listdir(makeUTF8bytes(download_dir)[0])
...
def processDestination(pp_path=None, dest_path=None, authorname=None, bookname=None, global_name=None, bookid=None,
    ...
    if booktype == 'ebook' and len(lazylibrarian.CONFIG['IMP_CALIBREDB']):
        ...
        try:
            ...
            if os.path.isdir(calibre_dir):  # assumed author directory
                ...
                if os.path.isdir(target_dir):
                    ...
                    if newbookfile:
                        setperm(target_dir)
                        for fname in os.listdir(makeUTF8bytes(target_dir)[0]):
                            fname = makeUnicode(fname)
        ...
        pp_path, encoding = makeUTF8bytes(pp_path)
        if encoding and lazylibrarian.LOGLEVEL & lazylibrarian.log_postprocess:
        ...
        for fname in os.listdir(pp_path):
            fname, encoding = makeUTF8bytes(fname)
            if encoding and lazylibrarian.LOGLEVEL & lazylibrarian.log_postprocess:
...
def processAutoAdd(src_path=None, booktype='book'):
    ...
    try:
        names = os.listdir(makeUTF8bytes(src_path)[0])
